...
public class QuadNode {
    ...
    public HashMap<Integer, QuadNode> split() {
        ...
        boolean h = (area.height%2==0);
        boolean w = (area.width%2==0);
        HashMap<Integer, QuadNode> children = new HashMap<Integer, QuadNode>();
        int childrenPosition = generatePosition(NW);
        children.put(childrenPosition, new QuadNode(this.tree, new Rectangle(area.x, area.y, width+(w?1:0), height+(h?1:0)), childrenPosition, this.depth + 1, generateNeighboursSizes(NW)));
        childrenPosition = generatePosition(SW);
        children.put(childrenPosition, new QuadNode(this.tree, new Rectangle(area.x, area.y + height, width+(w?1:0), height), childrenPosition, this.depth + 1, generateNeighboursSizes(SW)));
        childrenPosition = generatePosition(NE);
        children.put(childrenPosition, new QuadNode(this.tree, new Rectangle(area.x + width, area.y, width, height+(h?1:0)), childrenPosition, this.depth + 1, generateNeighboursSizes(NE)));
