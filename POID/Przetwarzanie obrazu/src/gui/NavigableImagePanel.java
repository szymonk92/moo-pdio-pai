...
public final class NavigableImagePanel extends JPanel implements PropertyChangeListener {
    ...
    /**
     * <p>Identifies a change to the quality.</p>
     */
    public static final String QUALITY_LEVEL_CHANGED_PROPERTY = "qualityLevel";
    /**
    ...
    public static class ZoomDevice {
         ...
         */
        public static final NavigableImagePanel.ZoomDevice NONE = new NavigableImagePanel.ZoomDevice("none");
        /**
         * <p>Identifies the left and right mouse buttons as the zooming
         * device.</p>
         */
        public static final NavigableImagePanel.ZoomDevice MOUSE_BUTTON = new NavigableImagePanel.ZoomDevice("mouseButton");
        /**
         * <p>Identifies the mouse scroll wheel as the zooming device.</p>
         */
        public static final NavigableImagePanel.ZoomDevice MOUSE_WHEEL = new NavigableImagePanel.ZoomDevice("mouseWheel");
    ...
    public NavigableImagePanel() {
        ...
        setZoomDevice(NavigableImagePanel.ZoomDevice.MOUSE_WHEEL);
    }
...
public void setTabData(TabData data){
    ...
    private void addButtonZoomDevice() {
        if (buttonZoomDevice == null) {
            buttonZoomDevice = new NavigableImagePanel.ButtonZoomDevice();
    ...
    public void setZoomDevice(NavigableImagePanel.ZoomDevice newZoomDevice) {
        if (newZoomDevice == NavigableImagePanel.ZoomDevice.NONE) {
            removeWheelZoomDevice();
            removeButtonZoomDevice();
        } else if (newZoomDevice == NavigableImagePanel.ZoomDevice.MOUSE_BUTTON) {
            removeWheelZoomDevice();
            addButtonZoomDevice();
        } else if (newZoomDevice == NavigableImagePanel.ZoomDevice.MOUSE_WHEEL) {
            removeButtonZoomDevice();
            addWheelZoomDevice();
        }
    }
     ...
     */
    public NavigableImagePanel.ZoomDevice getZoomDevice() {
        if (buttonZoomDevice != null) {
            return NavigableImagePanel.ZoomDevice.MOUSE_BUTTON;
        } else if (wheelZoomDevice != null) {
            return NavigableImagePanel.ZoomDevice.MOUSE_WHEEL;
        } else {
            return NavigableImagePanel.ZoomDevice.NONE;
        }
    }
    ...
    private NavigableImagePanel.Coords panelToImageCoords(Point p) {
        return new NavigableImagePanel.Coords((p.x - originX) / scale, (p.y - originY) / scale);
    }

    //Converts the original image coordinates into this panel's coordinates
    private NavigableImagePanel.Coords imageToPanelCoords(NavigableImagePanel.Coords p) {
        return new NavigableImagePanel.Coords((p.x * scale) + originX, (p.y * scale) + originY);
    }
    ...
    private boolean isInImage(Point p) {
        NavigableImagePanel.Coords coords = panelToImageCoords(p);
    ...
    /**
     * <p>Indicates whether the zoom of navigable image feature is enabled.</p>
     *
     * @return true if zoom of navigable image is enabled, false otherwise.
     */
    public boolean isZoomNavigableImageEnable() {
        return zoomNavigableImageEnable;
    }

    /**
     * <p>Enables/disables zoom of navigable image.</p>
     *
     * @param enabled enables/disables zoom of navigable image
     */
    public void setZoomNavigableImageEnable(boolean enabled) {
        zoomNavigableImageEnable = enabled;
    }
    ...
    public void setHighQualityRenderingEnabled(boolean enabled) {
        if (highQualityRenderingEnabled != enabled) {
            firePropertyChange(QUALITY_LEVEL_CHANGED_PROPERTY, highQualityRenderingEnabled,enabled);
        }
    ...
    public void setZoom(double newZoom, Point zoomingCenter) {
        NavigableImagePanel.Coords imageP = panelToImageCoords(zoomingCenter);
        if (imageP.x < 0.0) {
            imageP.x = 0.0;
        }
        if (imageP.y < 0.0) {
        ...
        NavigableImagePanel.Coords correctedP = imageToPanelCoords(imageP);
        double oldZoom = getZoom();
        scale = zoomToScale(newZoom);
        NavigableImagePanel.Coords panelP = imageToPanelCoords(imageP);
    ...
    private void zoomImage() {
        NavigableImagePanel.Coords imageP = panelToImageCoords(mousePosition);
        double oldZoom = getZoom();
        scale *= zoomFactor;
        NavigableImagePanel.Coords panelP = imageToPanelCoords(imageP);
    ...
    private Rectangle getImageClipBounds() {
        NavigableImagePanel.Coords startCoords = panelToImageCoords(new Point(0, 0));
        NavigableImagePanel.Coords endCoords = panelToImageCoords(new Point(getWidth() - 1, getHeight() - 1));
     ...
     *
     * @param g the
     * <code>Graphics</code> context for painting
     */
    @Override
